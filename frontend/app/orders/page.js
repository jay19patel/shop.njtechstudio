...
// ── Status helpers ─────────────────────────────────────────────────────────
const STATUS_CONFIG = {
  pending:             { icon: Clock,        color: 'bg-slate-100 text-orange-600 border-orange-100',  label: 'Pending' },
  processing:          { icon: Clock,        color: 'bg-yellow-50 text-yellow-600 border-yellow-100',  label: 'Processing' },
  shipped:             { icon: Truck,        color: 'bg-blue-50 text-slate-900 border-blue-100',        label: 'Shipped' },
        ...
        <div className="flex flex-col gap-8 md:gap-12">
          ...
          <div className="flex flex-col gap-4">
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight uppercase text-blue-950">
              My <span className="text-slate-900">Orders.</span>
            </h1>
          ...
          {!isAuthenticated && (
            ...
            <form onSubmit={handleManualSearch} className="flex flex-col sm:flex-row gap-3 bg-white p-3 rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 transition-all focus-within:shadow-blue-100">
              ...
              <button
                type="submit"
                disabled={loading}
                className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-100 hover:bg-slate-900/90 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 whitespace-nowrap"
              >
                {loading ? 'Searching...' : (
                  <>
              ...
              {orders.length === 0 ? (
                <div className="bg-white rounded-[40px] p-16 text-center flex flex-col items-center gap-8 border border-slate-100 shadow-xl shadow-slate-200/40 animate-in fade-in zoom-in duration-500">
                  ...
                  <div className="flex flex-col gap-3">
                    <h3 className="text-2xl font-extrabold tracking-tight uppercase text-blue-950">No orders found</h3>
                      ...
                      <div className="p-8 md:p-10 flex flex-col gap-8">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-6">
                          <div className="flex items-center gap-8">
                             ...
                             <div className="w-px h-8 bg-slate-50 hidden sm:block" />
                             <div className="flex flex-col gap-1">
                        ...
                        <div className="flex flex-col gap-6">
                          {(order.items || []).map((item, idx) => (
                            <div key={idx} className="flex items-center gap-6 group/item">
                              <div className="w-20 h-20 rounded-2xl bg-white border border-slate-100 overflow-hidden flex-shrink-0 transition-transform group-hover/item:scale-105">
                                {item.image ? (
                              ...
                              <div className="flex-grow flex flex-col gap-1">
                                <h4 className="text-base font-bold text-blue-950 group-hover/item:text-slate-900 transition-colors uppercase">{item.name}</h4>
                        ...
                        <div className="flex justify-between items-center">
                           ...
                           <Link
                             ...
                             className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-900 hover:text-slate-900 transition-all group-hover:translate-x-1"
                           >
                             View Full Details
          ...
          {!isAuthenticated && !searched && !loading && (
            <div className="bg-white rounded-[40px] p-16 text-center flex flex-col items-center gap-6 border border-slate-100 shadow-lg shadow-slate-200/40">
              ...
              <p className="text-slate-500 font-medium max-w-xs mx-auto">
                Already have an account? <Link href="/login" className="text-slate-900 font-bold hover:underline">Log in</Link> to see your orders automatically.
              </p>
            </div>
          )}
        </div>
